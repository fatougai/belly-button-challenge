...
// Function to generate the gauge chart
function generateGaugeChart(washingFrequency) {
  let gaugeData = [
    {
      domain: { x: [0, 1], y: [0, 1] },
...
function displayMetadata(sample) {
  ...
  // Check if metadata is found and not undefined
  if (metadataResult) {
    let sampleMetadata = d3.select("#sample-metadata");
    sampleMetadata.html("");
    ...
    // If metadata is not found, display an error message or handle it accordingly
    console.log("Metadata not found for selected sample.");
    let sampleMetadata = d3.select("#sample-metadata");
    sampleMetadata.html("<p>Metadata not found for selected sample.</p>");
  }
}
...
function init()  {
  ...
  generateBarChart(samples[0]);
  generateBubbleChart(samples[0]);
  generateGaugeChart(samples[0].wfreq); // Assuming washing frequency is stored in 'wfreq' property
  generatePieChart(samples[0]);
  ...
  function optionChanged(selectedSample) {
    ...
    if (selectedData) {
      generateBarChart(selectedData);
      generateBubbleChart(selectedData);
      generateGaugeChart(selectedData.wfreq); // Assuming washing frequency is stored in 'wfreq' property
      generatePieChart(selectedData);
      displayMetadata(selectedSample);
    } else {
      // If selectedData is not found, display an error message or handle it accordingly
      console.log("Data not found for selected sample.");
    }
  }
  function init() {
  ...
  dropdown.on("change", function() {
    let selectedSample = dropdown.property("value");
    optionChanged(selectedSample); // Call the optionChanged function when the dropdown changes
  });
}
});
